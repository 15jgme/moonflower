<script lang="ts">
	...
	async function fetchLastPaper(): Promise<Record> {
		const record = await pb.collection('users').getOne($currentUser.id, {
			expand: 'lastPickedPost',
			$autoCancel: false
		});
		return record.expand.lastPickedPost;
	}
</script>
...
<div>
	...
	{:else if $currentState == stateEnum.AWAITTIMER}
		{#await fetchLastPaper()}
			<button
				class="p-4 text-white bg-orange-700 hover:bg-orange-800 focus:ring-4 focus:ring-orange-300 font-medium rounded-lg text-sm px-5 py-2.5  dark:bg-orange-400 dark:hover:bg-orange-500 dark:focus:ring-orange-800 w-auto md:w-36"
				disabled
				><svg class="animate-spin h-5 w-5 mr-3 fill-white" viewBox="0 0 24 24">
					<path
						d="M10.14,1.16a11,11,0,0,0-9,8.92A1.59,1.59,0,0,0,2.46,12,1.52,1.52,0,0,0,4.11,10.7a8,8,0,0,1,6.66-6.61A1.42,1.42,0,0,0,12,2.69h0A1.57,1.57,0,0,0,10.14,1.16Z"
